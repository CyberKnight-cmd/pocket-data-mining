...
impl HuimAlgorithm for Tko {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        ...
        let tko_state = if let Some(k) = ctx.k {
            Some(Arc::new(TkoState::new(k as usize)))
        } else {
            None
        };
        ...
        ctx.execute_tasks(task_indices, |i, writer| {
            ...
            let mut current_thresh = if let Some(state) = &tko_state { state.get() } else { ctx.min_utility };
            ...
            if !extensions.is_empty() {
                tko_search(&[item_x], body_x, extensions, &eucs_arc, writer, ctx, tko_state.clone()).unwrap();
            }
        });
...
fn tko_search(
    ...
    for i in 0..extensions.len() {
        ...
        if !next_extensions.is_empty() {
            tko_search(&itemset_px, &body_px_entries, next_extensions, eucs, writer, ctx, tko_state.clone())?;
        }
    }
    Ok(())
}
