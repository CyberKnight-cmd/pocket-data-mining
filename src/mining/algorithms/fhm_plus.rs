...
/// FHM+ mining engine. Implements exact HUIM with EUCS pruning and Length Constraints.
pub struct FhmPlus {
...
impl HuimAlgorithm for FhmPlus {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        let dataset_path = source.expect_file("FHM+");
        use std::fs::File;
        ...
        // Pass 2: build 1-itemset utility lists and EUCS
        ...
        ctx.execute_tasks(task_indices, |i, writer| {
            ...
            if ul_x.is_high_utility(ctx.min_utility) && 1 >= ctx.min_length && 1 <= ctx.max_length {
            ...
            for j in (i+1)..filtered.len() {
                ...
                if eucs_arc.can_prune(item_x, item_y, ctx.min_utility) {
                    continue;
                }
            ...
            if !extensions.is_empty() {
                fhm_plus_search(&[item_x], body_x, extensions, &eucs_arc, writer, ctx).unwrap();
            }
        });
...
fn fhm_plus_search(
    ...
    for i in 0..extensions.len() {
        ...
        if ul_px.is_high_utility(ctx.min_utility) && itemset_px.len() >= ctx.min_length && itemset_px.len() <= ctx.max_length {
            writer.write_hui(&itemset_px, ul_px.sum_iutils)?;
            ctx.progress.huis_found.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        }

        if itemset_px.len() >= ctx.max_length {
            continue;
        }
        ...
        for j in (i+1)..extensions.len() {
            ...
            // EUCS pruning
            if eucs.can_prune(item_x, item_y, ctx.min_utility) {
                continue;
            }
