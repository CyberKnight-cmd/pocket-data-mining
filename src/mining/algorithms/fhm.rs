...
/// FHM mining engine. Implements exact HUIM with EUCS pruning,
/// all multi-level UL reads going through the buffer pool.
pub struct Fhm {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        ...
        ctx.execute_tasks(task_indices, |i, writer| {
            ...
            if !extensions.is_empty() {
                fhm_search(&[item_x], body_x, extensions, &eucs_arc, writer, ctx).unwrap();
            }
        });
