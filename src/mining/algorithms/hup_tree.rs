...
impl NodeArena {
    fn new(pool: Arc<BufferPool>, store: Arc<dyn ChunkStore + Send + Sync>, progress: Arc<MiningProgress>) -> Self {
        ...
        let nodes_per_page = page_size / std::mem::size_of::<HupNode>();
        Self {
            pool,
            store,
            pages: Vec::new(),
    ...
    fn get_node(&self, ptr: u32) -> io::Result<HupNode> {
        let page_index = (ptr as usize) / self.nodes_per_page;
        let offset = (ptr as usize) % self.nodes_per_page;
        let page_id = self.pages[page_index];
        ...
        let nodes_ptr = bytes.as_ptr() as *const HupNode;
        let nodes = unsafe { std::slice::from_raw_parts(nodes_ptr, self.nodes_per_page) };
        Ok(nodes[offset])
    }

    fn set_node(&self, ptr: u32, node: HupNode) -> io::Result<()> {
        let page_index = (ptr as usize) / self.nodes_per_page;
        let offset = (ptr as usize) % self.nodes_per_page;
        let page_id = self.pages[page_index];
        let guard = self.pool.pin_mut(page_id)?;
...
fn mine_tree(tree: &HupTreeStruct, arena: &mut NodeArena, prefix: &[ItemId], min_utility: Utility, candidates: &mut Vec<Vec<ItemId>>, ctx: &MiningContext) -> io::Result<()> {
    ...
    for &item in tree.items_order.iter().rev() {
        ...
        let mut cond_tree = HupTreeStruct::new(arena, local_items.clone())?;
        for (path, twu) in cpb {
...
impl HuimAlgorithm for HupTree {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        ...
        let mut tree = HupTreeStruct::new(&mut arena, valid_items.clone())?;
