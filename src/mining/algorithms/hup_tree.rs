...
fn mine_tree(tree: &HupTreeStruct, prefix: &[ItemId], min_utility: Utility, candidates: &mut Vec<Vec<ItemId>>, progress: &std::sync::Arc<crate::progress::MiningProgress>) {
    ...
    for &item in tree.items_order.iter().rev() {
        ...
        progress.fast_path_reads.fetch_add(reads, std::sync::atomic::Ordering::Relaxed);
        ...
        mine_tree(&cond_tree, &new_prefix, min_utility, candidates, progress);
    }
}
...
impl HuimAlgorithm for HupTree {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        ...
        mine_tree(&tree, &[], ctx.min_utility, &mut all_candidates, &ctx.progress);
