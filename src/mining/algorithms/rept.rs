...
impl HuimAlgorithm for Rept {
    ...
    fn run(&mut self, source: DataSource, ctx: &mut MiningContext) -> io::Result<u64> {
        let dataset_path = source.expect_file("Rept");
        use std::fs::File;
        ...
        let rept_state = if let Some(k) = ctx.k {
            Some(Arc::new(ReptState::new(k as usize)))
        } else {
            None
        };
        ...
        ctx.execute_tasks(task_indices, |i, writer| {
            ...
            if !extensions.is_empty() {
                rept_search(&[item_x], body_x, extensions, &eucs_arc, writer, ctx, rept_state.clone()).unwrap();
            }
        });
...
fn rept_search(
    ...
    ctx: &MiningContext,
    rept_state: Option<Arc<ReptState>>,
) -> io::Result<()> {
    ...
    for i in 0..extensions.len() {
        ...
        if !next_extensions.is_empty() {
            rept_search(&itemset_px, &body_px_entries, next_extensions, eucs, writer, ctx, rept_state.clone())?;
        }
    }
    Ok(())
}
